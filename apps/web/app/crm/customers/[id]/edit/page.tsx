  ...
  useEffect(() => {
    if (customer) {
      setFormData({
        ...
        tagIds: customer.tags ? customer.tags.map((t: any) => t.tag.id) : [],
      });
    }
  }, [customer]);

  const updateMutation = useMutation({
    mutationFn: (payload: any) => crmService.updateCustomer(customerId, payload),
    onSuccess: () => {
        ...
        <main className="flex-1 p-8 max-w-4xl mx-auto space-y-6 w-full">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Update customer profile details.</p>
            </div>
          </div>
