...
export default function SupplierDetailsPage() {
  ...
  const { data: activitiesData } = useQuery({
    ...
    queryFn: () => crmService.getSupplierActivities(supplierId),
    enabled: Boolean(supplierId && supplierId !== 'undefined'),
  });
  ...
  const noteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['supplier-activities', supplierId] });
    },
  });

  const deleteNoteMutation = useMutation({
    mutationFn: (noteId: string) => crmService.deleteSupplierNote(supplierId, noteId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['supplier', supplierId] });
    },
  });
  ...
  const addAddressMutation = useMutation({
    mutationFn: (payload: any) => crmService.addSupplierAddress(supplierId, payload),
    onSuccess: () => {
  ...
  const deleteAddressMutation = useMutation({
    mutationFn: (addressId: string) => crmService.deleteSupplierAddress(supplierId, addressId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['supplier', supplierId] });
    },
  });
  ...
  const archiveMutation = useMutation({
    mutationFn: () => crmService.archiveSupplier(supplierId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['supplier', supplierId] });
  ...
  const restoreMutation = useMutation({
    mutationFn: () => crmService.restoreSupplier(supplierId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['supplier', supplierId] });
      queryClient.invalidateQueries({ queryKey: ['suppliers-list'] });
    },
      ...
      <div className="min-h-screen bg-background flex flex-col">
        ...
        <div className="flex flex-1">
          ...
          <main className="flex-1 p-8 text-center space-y-4">
            ...
            <h2 className="text-xl font-bold">Supplier profile not found</h2>
            <Link href="/crm/suppliers">
              <Button size="sm">Back to Suppliers</Button>
            </Link>
          </main>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 p-8 space-y-6 max-w-6xl mx-auto w-full">
          ...
          {/* Profile Header */}
          <Card className="border-border/80 p-6 bg-gradient-to-r from-amber-500/5 via-card to-card">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">
                  ...
                  {supplier.creator?.name && ` by ${supplier.creator.name}`}
                </p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Link href={`/crm/suppliers/${supplier.id}/edit`}>
            ...
            {supplier.tags && supplier.tags.length > 0 && (
              <div className="flex flex-wrap gap-1.5 pt-4 border-t border-border/40 mt-4">
                {supplier.tags.map((t: any) => (
                  <span
                    key={t.tag.id}
                    className="text-[11px] px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-600 dark:text-amber-400 font-medium border border-amber-500/20"
                  >
                    {t.tag.name}
                  </span>
                ))}
          ...
          {activeTab === 'info' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card className="border-border/80">
                ...
                <CardContent className="space-y-4 text-xs">
                  <div>
                    ...
                    <p className="font-semibold text-sm text-foreground mt-0.5">{supplier.phone}</p>
                  </div>
                  {supplier.alternatePhone && (
                    <div>
                      <span className="text-muted-foreground">Alternate Phone:</span>
                      <p className="font-medium text-foreground mt-0.5">{supplier.alternatePhone}</p>
                    </div>
                  )}
                  <div>
                    <span className="text-muted-foreground">Email:</span>
                    <p className="font-medium text-foreground mt-0.5">{supplier.email || '—'}</p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="border-border/80">
                <CardHeader>
                  <CardTitle className="text-base">Tax & Accounts Info</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4 text-xs">
                  <div>
                    <span className="text-muted-foreground">GST Number:</span>
                    <p className="font-mono text-sm text-foreground mt-0.5">{supplier.gstNumber || 'Not provided'}</p>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Tax ID / PAN:</span>
                    <p className="font-mono text-sm text-foreground mt-0.5">{supplier.taxId || 'Not provided'}</p>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Total Purchases:</span>
                    <p className="font-semibold text-foreground mt-0.5">{supplier.totalPurchasesDisplay || '—'}</p>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Outstanding Balance:</span>
                    <p className="font-semibold text-foreground mt-0.5">{supplier.outstandingBalanceDisplay || '₹0.00'}</p>
                  </div>
                </CardContent>
              </Card>
            </div>
                ...
                <Card className="border-amber-500/40 bg-amber-500/5">
                  ...
                  <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1 flex items-center gap-2 pt-5">
                      ...
                      />
                      <label htmlFor="isDefaultSupp" className="text-xs font-medium">Set as Default</label>
                    </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {supplier.addresses && supplier.addresses.length > 0 ? (
                  supplier.addresses.map((addr: any) => (
                    <Card key={addr.id} className="border-border/80">
            ...
            <div className="space-y-6">
              <Card className="border-border/80">
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm">Add Supplier Note</CardTitle>
                </CardHeader>
              ...
              <div className="space-y-3">
                {supplier.notesList && supplier.notesList.length > 0 ? (
                  supplier.notesList.map((n: any) => (
            ...
            <Card className="border-border/80 p-6">
              <CardHeader className="pb-4 pt-0 px-0">
                <CardTitle className="text-base flex items-center gap-2">
                  <Clock className="h-4 w-4 text-amber-500" /> Activity History
                </CardTitle>
              </CardHeader>
                  ...
                  <div className="relative pl-6 space-y-6 before:absolute before:left-2 before:top-2 before:bottom-2 before:w-0.5 before:bg-border/60">
                    {activities.map((act: any) => (
                      <div key={act.id} className="relative flex items-start gap-3 text-xs">
                        <div className="absolute left-[-1.15rem] top-1 w-3 h-3 rounded-full bg-amber-500 ring-4 ring-background" />
